<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:variable name="resource-id">
        <xsl:choose>
            ...
            <xsl:when
                test="/descendant::t:publicationStmt/t:idno[@type = 'URI'][starts-with(., $base-uri)]">
                <xsl:value-of
                    select="replace(replace(/descendant::t:publicationStmt/t:idno[@type = 'URI'][starts-with(., $base-uri)][1], '/tei', ''), '/source', '')"
                />
            </xsl:when>
            <xsl:when test="/descendant::t:publicationStmt/t:idno[@type = 'URI']">
                <xsl:value-of
                    select="replace(replace(/descendant::t:publicationStmt/t:idno[@type = 'URI'][1], '/tei', ''), '/source', '')"
                />
            </xsl:when>
            <xsl:when test="/descendant::t:idno[@type = 'URI'][starts-with(., $base-uri)]">
                <xsl:value-of
                    select="replace(replace(/descendant::t:idno[@type = 'URI'][starts-with(., $base-uri)][1], '/tei', ''), '/source', '')"
                />
            </xsl:when>
            <xsl:otherwise>
        ...
        <div id="about">
            <xsl:choose>
                <xsl:when test="contains($resource-id, '/bibl/')">
                    ...
                    <xsl:apply-templates
                        select="descendant-or-self::t:teiHeader/t:fileDesc/t:titleStmt"
                        mode="about-bibl"/>
                </xsl:when>
                <xsl:otherwise>
                    <h3>About this Entry</h3>
                    <xsl:apply-templates
                        select="descendant-or-self::t:teiHeader/t:fileDesc/t:titleStmt" mode="about"
                    />
                </xsl:otherwise>
            </xsl:choose>
        </div>
                ...
                <li>
                    <xsl:if test="descendant::t:lang/text()">
                        <span class="tei-label">
                            <xsl:value-of
                                select="local:expand-lang(descendant::t:lang[1]/text(), 'lawd:Edition')"
                            />: </span>
                    </xsl:if>
                    <span>
                        ...
                        <xsl:if test="t:listRelation/t:relation">
                            ...
                            <xsl:variable name="bibl-type"
                                select="local:translate-label(string(@type), 0)"/>
                            <xsl:for-each
                                select="t:listRelation/t:relation[not(@ref = 'lawd:embodies')]">
                                ...
                                <xsl:variable name="all-bibs">
                                    <bibs xmlns="http://www.tei-c.org/ns/1.0">
                                        <xsl:for-each-group
                                            select="ancestor::t:bibl[@type = 'lawd:ConceptualWork']/t:bibl"
                                            group-by="@type">
                                            <bibList>
                                                <xsl:for-each select="current-group()">
                                                  <bibl bibid="{@xml:id}" position="{position()}"
                                                  type="{local:translate-label(string(current-grouping-key()),count(current-group()))}"
                                                  ref="{string-join(child::t:ptr/@target,' ')}"/>
                                                </xsl:for-each>
                                            </bibList>
                                        </xsl:for-each-group>
                                    </bibs>
                                        ...
                                        <xsl:when test="@ref = 'lawd:hasCitation'">
                                            <bib-relations xmlns="http://www.tei-c.org/ns/1.0">
                                                <xsl:for-each select="tokenize(@active, ' ')">
                                                  ...
                                                  select="replace(., '#', '')"/>
                                                  <xsl:for-each
                                                  select="$all-bibs/descendant::t:bibl[@bibid = $bibl-id]">
                                                  <xsl:copy-of select="."/>
                                                  </xsl:for-each>
                                                  </xsl:when>
                                                  <xsl:otherwise>
                                                  ...
                                                  <xsl:for-each
                                                  select="$all-bibs/descendant::t:bibl[@ref = $bibl-id]">
                                                  <xsl:copy-of select="."/>
                                                  </xsl:for-each>
                                                  </xsl:otherwise>
                                                  </xsl:choose>
                                ...
                                <xsl:for-each-group select="$bibl-rel/descendant-or-self::t:bibl"
                                    group-by="@type">
        ...
        <xsl:choose>
            <xsl:when test="parent::t:body">
                <div class="well preferred-citation">
                    ...
                    <xsl:apply-templates select="self::*" mode="bibliography"/>. </div>
    ...
    <xsl:template name="citationInfo">
        <div class="citationinfo">
            ...
            <div id="citation-note" class="well">
                <xsl:apply-templates select="descendant-or-self::t:teiHeader/t:fileDesc/t:titleStmt"
                    mode="cite-foot"/>
                <div class="collapse" id="showcit">
                    ...
                    <div id="license">
                        ...
                        <div>
                            ...
                            <xsl:choose>
                                <xsl:when
                                    test="descendant-or-self::t:teiHeader/t:fileDesc/t:publicationStmt/t:date[1]/text() castable as xs:date">
                                    <xsl:value-of
                                        select="format-date(xs:date(//t:teiHeader/t:fileDesc/t:publicationStmt/t:date[1]), '[Y]')"
                                    />
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of
                                        select="descendant-or-self::t:teiHeader/t:fileDesc/t:publicationStmt/t:date[1]"
                                    />
                                </xsl:otherwise>
                            </xsl:choose>. </div>
                        <xsl:apply-templates
                            select="descendant-or-self::t:teiHeader/t:fileDesc/t:publicationStmt/t:availability/t:licence"
                        />
                    </div>
                </div>
                <a class="togglelink pull-right btn-link" data-toggle="collapse"
                    data-target="#showcit" data-text-swap="Hide citation">Show full citation
                    information...</a>
            </div>
        </div>
    </xsl:template>
    ...
    <xsl:template match="t:div | t:div1 | t:div2 | t:div3 | t:div4 | t:div5">
        ...
        <xsl:variable name="currentid"
            select="
                concat(if ($parentID != '') then
                    $parentID
                else
                    'id', '.', @n)"/>
        <div
            class="{concat('tei-',name(.))}{if(@unit) then concat(' tei-',@unit) else ()} {if(@type) then concat(' tei-',@type) else ()}">
            <xsl:choose>
    ...
    <xsl:template match="t:head">
        <xsl:choose>
            <xsl:when test="parent::t:div1">
                <h2
                    class="tei-head {if(parent::*[1]/@type) then concat(' tei-',parent::*[1]/@type) else ()}">
                    <xsl:sequence select="local:attributes(.)"/>
                    <xsl:apply-templates/>
                </h2>
            </xsl:when>
            ...
            <xsl:otherwise>
                <span
                    class="{concat('tei-',name(parent::*[1]))} {if(parent::*[1]/@type) then concat(' tei-',parent::*[1]/@type) else ()} tei-head">
                    <xsl:sequence select="local:attributes(.)"/>
                    <xsl:apply-templates/>
                </span>
            </xsl:otherwise>
    ...
    <xsl:template match="t:location">
        <xsl:choose>
            ...
            <xsl:when test=".[@type = 'gps' and t:geo]">
                <li>Coordinates: <ul class="unstyled offset1">
                        <li>
    ...
    <xsl:template match="t:note">
        ...
        <xsl:choose>
            ...
            <xsl:when test="@type = 'deprecation'">
                <li>
                    <span>
                        <xsl:apply-templates select="../t:link[contains(@target, $xmlid)]"/>: <xsl:apply-templates/>
                ...
                <li class="note">
                    <xsl:if test="descendant::t:lang/text()">
                        <span class="srp-label">
                            <xsl:value-of
                                select="local:expand-lang(descendant::t:lang/text(), 'ancientVersion')"
                            />: </span>
                    </xsl:if>
                    <span>
                        <xsl:sequence select="local:attributes(.)"/>
            ...
            <xsl:when test="@type = 'modernTranslation'">
                <li>
                    <xsl:if test="descendant::t:lang/text()">
                        <span class="srp-label">
                            <xsl:value-of
                                select="local:expand-lang(descendant::t:lang/text(), 'modernTranslation')"
                            />: </span>
                    </xsl:if>
                    <span>
                        <xsl:sequence select="local:attributes(.)"/>
                ...
                <li>
                    <span>
                        ...
                        <xsl:if test="t:bibl/@corresp">
                            ...
                            <xsl:text> (</xsl:text>
                            <xsl:choose>
                                <xsl:when test="@ana = 'partialTranslation'">Partial
                                    edition</xsl:when>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:variable name="corresp"
                                        select="substring-after(t:bibl/@corresp, '#')"/>
            ...
            <xsl:when
                test="@rend = ('footer', 'foot', 'footnote') or @place = ('footer', 'foot', 'footnote')">
        ...
        <p class="footnote-text">
            <xsl:if test="@n">
                ...
                <span class="notes footnote-refs">
                    <span class="footnote-ref">‎<xsl:value-of select="@n"/></span>
                </span>
            </xsl:if>
            <xsl:choose>
                <xsl:when test="t:quote">
        ...
        <xsl:variable name="currentid"
            select="
                concat(if ($parentID != '') then
                    ...
                    'id', '.', @n)"/>
        <span
            class="{concat('tei-',name(.))}                           {if(@unit) then concat(' tei-',@unit) else ()}                           {if(@type) then concat(' tei-',@type) else ()}                          {if(self::t:l) then 'display' else ()}">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <span class="text-number badge {if(self::t:l) then 'display' else ()}">
                            <xsl:if test="self::t:pb">pb. </xsl:if>
                            <xsl:value-of select="@n"/>
                        </span>
                        <xsl:text> </xsl:text>
                    </xsl:otherwise>
        ...
        <xsl:variable name="currentid"
            select="
                concat(if ($parentID != '') then
                    ...
                    'id', '.', @n)"/>
        <p
            class="{concat('tei-',name(.))}{if(@unit) then concat(' tei-',@unit) else ()} text-display">
            <xsl:choose>
                <xsl:when test="child::t:head">
                    <xsl:attribute name="id">
                        <xsl:value-of select="concat('Head-', $currentid)"/>
    ...
    <xsl:template match="t:place | t:person | t:bibl[starts-with(@xml:id, 'work-')] | t:entryFree">
        <xsl:if test="not(empty(t:desc[not(starts-with(@xml:id, 'abstract'))][1]))">
            <div id="description">
                ...
                <ul>
                    <xsl:for-each-group select="t:desc"
                        group-by="
                            if (contains(@xml:lang, '-') = true()) then
                                substring-before(@xml:lang, '-')
                            else
                                @xml:lang">
                        <xsl:sort collation="{$languages}"
                            select="
                                if (contains(@xml:lang, '-') = true()) then
                                    substring-before(@xml:lang, '-')
                                else
        ...
        <xsl:if test="self::t:entryFree">
            <xsl:if
                test="t:desc[@type = 'abstract'] | t:desc[starts-with(@xml:id, 'abstract-en')] | t:note[@type = 'abstract']">
                <div style="margin-bottom:1em;">
                    <h3>Abstract</h3>
                    <xsl:apply-templates
                        select="t:desc[@type = 'abstract' or starts-with(@xml:id, 'abstract-en')][1] | t:note[@type = 'abstract']"
                    />
                </div>
            </xsl:if>
            <xsl:if test="t:term">
        ...
        <xsl:if test="self::t:person">
            <div>
                ...
                <xsl:if test="t:persName[not(empty(descendant-or-self::text()))]">
                    ...
                    <ul class="persNames">
                        <xsl:apply-templates
                            select="t:persName[@syriaca-tags = '#syriaca-headword' and starts-with(@xml:lang, 'syr')]"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select="t:persName[@syriaca-tags = '#syriaca-headword' and starts-with(@xml:lang, 'en')]"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select="t:persName[(not(@syriaca-tags) or @syriaca-tags != '#syriaca-headword') and starts-with(@xml:lang, 'syr')]"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates select="t:persName[starts-with(@xml:lang, 'ar')]"
                            mode="list">
                            <xsl:sort lang="ar" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select="t:persName[(not(@syriaca-tags) or @syriaca-tags != '#syriaca-headword') and not(starts-with(@xml:lang, 'syr') or starts-with(@xml:lang, 'ar')) and not(@syriaca-tags = '#syriaca-simplified-script')]"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                    </ul>
        ...
        <xsl:if
            test="self::t:bibl[starts-with(@xml:id, 'work-')] and t:title[not(@type = ('initial-rubric', 'final-rubric', 'abbreviation'))]">
            <div class="well">
                <h3>Titles</h3>
                <ul>
                    <xsl:for-each
                        select="t:title[(not(@type) or not(@type = ('initial-rubric', 'final-rubric', 'abbreviation'))) and not(@syriaca-tags = '#syriaca-simplified-script')]">
                        <xsl:apply-templates
                            select=".[contains(@syriaca-tags, '#syriaca-headword') and starts-with(@xml:lang, 'en')]"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select=".[contains(@syriaca-tags, '#syriaca-headword') and starts-with(@xml:lang, 'syr')]"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select=".[(not(@syriaca-tags) or not(contains(@syriaca-tags, '#syriaca-headword'))) and starts-with(@xml:lang, 'syr')]"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        ...
                        <xsl:apply-templates
                            select=".[(not(@syriaca-tags) or not(contains(@syriaca-tags, '#syriaca-headword'))) and not(starts-with(@xml:lang, 'syr') or starts-with(@xml:lang, 'ar'))]"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                    </xsl:for-each>
                ...
                <xsl:if
                    test="not(empty(t:desc[@type = 'abstract' or starts-with(@xml:id, 'abstract-en')][1] | t:note[@type = 'abstract']))">
                    <h3>Abstract</h3>
                    <xsl:apply-templates
                        select="t:desc[@type = 'abstract' or starts-with(@xml:id, 'abstract-en')][1] | t:note[@type = 'abstract']"
                    />
                ...
                <xsl:if test="@ana">
                    <xsl:for-each select="tokenize(@ana, ' ')">
                        <xsl:variable name="filepath">
                            <xsl:value-of
                                select="concat('xmldb:exist://', substring-before(replace(., $base-uri, $nav-base), '#'))"
                            />
                        </xsl:variable>
                        ...
                        <xsl:if test="doc-available($filepath)">
                            <p>
                                ...
                                <xsl:for-each
                                    select="document($filepath)/descendant::t:*[@xml:id = $ana-id]">
                        ...
                        <xsl:for-each select="t:idno[contains(., $base-uri)]">
                            <xsl:choose>
                                ...
                                <xsl:when test="@type = 'BHSYRE'">
                                    <xsl:value-of
                                        select="concat(replace(@type, 'BHSYRE', 'BHS'), ': ', .)"/>
                                </xsl:when>
                                <xsl:otherwise>
        ...
        <xsl:if test="self::t:place">
            <xsl:if test="t:placeName">
                <div id="placenames" class="well">
                    ...
                    <ul>
                        <xsl:apply-templates
                            select="t:placeName[@syriaca-tags = '#syriaca-headword' and @xml:lang = 'syr']"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select="t:placeName[@syriaca-tags = '#syriaca-headword' and @xml:lang = 'en']"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                        <xsl:apply-templates
                            select="t:placeName[(not(@syriaca-tags) or @syriaca-tags != '#syriaca-headword') and starts-with(@xml:lang, 'syr')]"
                            mode="list">
                            <xsl:sort lang="syr" select="."/>
                        </xsl:apply-templates>
                        ...
                        <xsl:apply-templates
                            select="t:placeName[(not(@syriaca-tags) or @syriaca-tags != '#syriaca-headword') and not(starts-with(@xml:lang, 'syr') or starts-with(@xml:lang, 'ar')) and not(@syriaca-tags = '#syriaca-simplified-script')]"
                            mode="list">
                            <xsl:sort collation="{$mixed}" select="."/>
                        </xsl:apply-templates>
                    </ul>
                ...
                <xsl:if test="t:related-items/t:relation[contains(@uri, 'place')]">
                    <div>
                        <dl class="dl-horizontal dl-srophe">
                            <xsl:for-each-group
                                select="t:related-items/t:relation[contains(@uri, 'place')]"
                                group-by="@ref">
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when
                                        test="current-grouping-key() = ('born-at', 'syriaca:bornAt')">
                                        <dt>
                                            <i class="srophe-marker born-at"/>
                                        </dt>
                                    </xsl:when>
                                    <xsl:when
                                        test="current-grouping-key() = ('died-at', 'syriaca:diedAt')">
                                        <dt>
                                            <i class="srophe-marker died-at"/>
                                        </dt>
                                    </xsl:when>
                                    <xsl:when
                                        test="current-grouping-key() = ('has-literary-connection-to-place', 'syriaca:hasLiteraryConnectionToPlace')">
                                        <dt>
        ...
        <xsl:if test="t:state">
            <xsl:for-each-group
                select="//t:state[not(@when) and not(@notBefore) and not(@notAfter) and not(@to) and not(@from)]"
                group-by="@type">
                <h4>
                    <xsl:value-of
                        select="concat(upper-case(substring(current-grouping-key(), 1, 1)), substring(current-grouping-key(), 2))"
                    />
                </h4>
                <ul>
        ...
        <xsl:if test="t:event[@type = 'attestation']">
            <div id="attestation">
                <h3>Attestation<xsl:if test="count(t:event[@type = 'attestation']) &gt; 1"
                        >s</xsl:if>
                </h3>
                <ul>
        ...
        <xsl:if
            test="t:confessions/t:state[@type = 'confession'] | t:state[@type = 'confession'][parent::t:place]">
            <div>
                <h3>Known Religious Communities</h3>
                <p class="caveat">
                    <em>This list is not necessarily exhaustive, and the order does not represent
                        importance or proportion of the population. Dates do not represent starting
                        or ending dates of a group's presence, but rather when they are attested.
                        Instead, the list only represents groups for which Syriaca.org has source(s)
                        and dates.</em>
                </p>
                <xsl:choose>
        ...
        <xsl:if test="t:note[not(@type = 'abstract')]">
            <xsl:variable name="rules"
                select="'&lt; prologue &lt; incipit &lt; explicit &lt;                  editions &lt; modernTranslation &lt;                  ancientVersion &lt; MSS'"/>
            <xsl:for-each-group select="t:note[not(@type = 'abstract')][exists(@type)]"
                group-by="@type">
                <xsl:sort select="current-grouping-key()"
                    collation="http://saxon.sf.net/collation?rules={encode-for-uri($rules)};ignore-case=yes;ignore-modifiers=yes;ignore-symbols=yes)"
                    order="ascending"/>
                <!--<xsl:sort select="current-grouping-key()" order="descending"/>-->
                <xsl:variable name="label">
                    <xsl:choose>
                        <xsl:when test="current-grouping-key() = 'MSS'">Syriac Manuscript
                            Witnesses</xsl:when>
                        <xsl:when test="current-grouping-key() = 'incipit'">Incipit (Opening
                            Line)</xsl:when>
                        <xsl:when test="current-grouping-key() = 'explicit'">Explicit (Closing
                            Line)</xsl:when>
                        <xsl:when test="current-grouping-key() = 'ancientVersion'">Ancient
                            Versions</xsl:when>
                        <xsl:when test="current-grouping-key() = 'modernTranslation'">Modern
                            Translations</xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="current-grouping-key()"/>
                        </xsl:otherwise>
                    </xsl:choose>
                        ...
                        <xsl:sort
                            select="
                                if (current-grouping-key() = 'MSS') then
                                    substring-after(t:bibl/@xml:id, '-') = ''
                                else
                                    if (current-grouping-key() = 'editions') then
                                        substring-after(t:bibl/@corresp, '-') = ''
                                    else
                                        if (@xml:lang) then
                                            local:expand-lang(@xml:lang, $label)
                                        else
                                            ."
                            order="ascending"/>
                        <xsl:sort
                            select="
                                if (current-grouping-key() = 'MSS' and (substring-after(t:bibl/@xml:id, '-') castable as xs:integer)) then
                                    xs:integer(substring-after(t:bibl/@xml:id, '-'))
                                else
                                    if (@xml:lang) then
                                        local:expand-lang(@xml:lang, $label)
                                    else
        ...
        <xsl:if test="t:bibl">
            <xsl:if
                test="self::t:bibl[@type = 'lawd:Citation' or @type = 'lawd:ConceptualWork'] or parent::t:body">
                <xsl:variable name="type-order"/>
                <xsl:for-each-group select="t:bibl[exists(@type)][@type != 'lawd:Citation']"
                    group-by="@type">
                    <xsl:sort select="local:bibl-type-order(current-grouping-key())"
                        order="ascending"/>
                    <xsl:variable name="label">
                        <xsl:variable name="l"
                            select="local:translate-label(current-grouping-key(), count(current-group()))"/>
                        <xsl:choose>
                            <xsl:when test="$l != ''">
                    ...
                    <ol>
                        ...
                        <xsl:if test="count(current-group()) &gt; 8">
                            <div class="collapse"
                                id="showMore-{local:bibl-type-order(current-grouping-key())}">
                            ...
                            <button class="btn btn-link bibl-show togglelink" data-toggle="collapse"
                                ...
                                data-text-swap="Hide">Show all <xsl:value-of
                ...
                <ul>
                    <xsl:for-each select="/child::*/t:nested-place">
                        <xsl:sort collation="{$mixed}"
                            select="t:placeName[@xml:lang = 'en'][1]/@reg"/>
                        <li>
        ...
        <xsl:if test="self::t:person/@ana = '#syriaca-saint'">
            <div>
                <h3>Lives</h3>
                <p> [Under preparation. Syriaca.org is preparing a database of Syriac saints lives,
                    Biblioteca Hagiographica Syriaca Electronica, which will include links to lives
                    for saints here.] </p>
            </div>
        </xsl:if>
    ...
    <xsl:template match="t:placeName | t:title | t:persName" mode="list">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="/descendant-or-self::t:link[substring-before(@target, ' ') = $nameID][contains(@target, 'deprecation')]"/>
                    ...
                    <xsl:variable name="label">
                        <xsl:variable name="labelString">
                            <xsl:choose>
                                ...
                                <xsl:otherwise> Relationship </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
                        <xsl:value-of
                            select="concat(upper-case(substring(concat(substring($labelString, 1, 1), replace(substring($labelString, 2), '(\p{Lu})', concat(' ', '$1'))), 1, 1)), substring(concat(substring($labelString, 1, 1), replace(substring($labelString, 2), '(\p{Lu})', concat(' ', '$1'))), 2))"
                        />
                    </xsl:variable>
                    <span class="srp-label">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div id="citation-note">
                    ...
                    <br/>
                    <a href="#" class="btn-sm btn-info togglelink pull-right" data-toggle="collapse"
                        data-target="#showFullCitation" data-text-swap="Hide Citation Styles"
                        style="margin-top:1em;">Show Citation Styles <i
                    ...
                    <xsl:non-matching-substring>
                        <xsl:choose>
                            <xsl:when test="matches(., '\p{IsSyriac}')">
                                <a href="https://sedra.bethmardutho.org/api/word/{.}.html"
                                    class="sedra">
                                    <xsl:value-of select="."/>
                                </a>
                            </xsl:when>
    ...
    <xsl:template name="h1">
        ...
        <div class="idno seriesStmt"
            style="margin:0; margin-top:-1em; margin-bottom: 1em; padding:1em; color: #999999;">
            ...
            <xsl:variable name="next-uri"
                select="replace($resource-id, $current-id, string($next-id))"/>
            <xsl:variable name="prev-uri"
                select="replace($resource-id, $current-id, string($prev-id))"/>
            <small>
                <span class="uri">
                    ...
                    <xsl:text> </xsl:text>
                    <button type="button" class="btn btn-default btn-xs" id="idnoBtn"
                        data-clipboard-action="copy" data-clipboard-target="#syriaca-id">
                ...
                <xsl:if test="t:seriesStmt/t:biblScope/t:title">
                    <span class="series pull-right"
                        style="margin-left:2em; padding-left:2em; display:inline">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test=". = 'A Guide to Syriac Authors'">
                                    ...
                                    <a href="{$nav-base}/authors/index.html">
                                        <span class="syriaca-icon syriaca-authors"
                                            style="font-size:1.35em; vertical-align: middle;">
                                            <span class="path1"/>
                                            <span class="path2"/>
                                            <span class="path3"/>
                                            <span class="path4"/>
                                ...
                                <xsl:when test=". = 'Bibliotheca Hagiographica Syriaca Electronica'">
                                    ...
                                    <a href="{$nav-base}/bhse/index.html">
                                        <span class="syriaca-icon syriaca-bhse"
                                            style="font-size:1.35em; vertical-align: middle;">
                                            <span class="path1"/>
                                            <span class="path2"/>
                                            <span class="path3"/>
                                ...
                                <xsl:when test=". = 'New Handbook of Syriac Literature'">
                                    ...
                                    <a href="{$nav-base}/nhsl/index.html">
                                        <span class="syriaca-icon syriaca-nhsl"
                                            style="font-size:1.35em; vertical-align: middle;">
                                            <span class="path1"/>
                                            <span class="path2"/>
                                            <span class="path3"/>
    ...
    <xsl:template name="title">
        <span id="title">
            <xsl:choose>
                <xsl:when test="contains($resource-id, '/subject/')"> Term: <xsl:apply-templates
                        select="descendant::*[contains(@syriaca-tags, '#syriaca-headword')][starts-with(@xml:lang, 'en')][not(empty(node()))][1]"
                        mode="plain"/>
                </xsl:when>
                <xsl:when test="descendant::*[contains(@syriaca-tags, '#syriaca-headword')]">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="descendant::*[contains(@syriaca-tags, '#anonymous-description')]">
                            <xsl:value-of
                                select="descendant::*[contains(@syriaca-tags, '#anonymous-description')][1]"
                            />
                        </xsl:when>
                        <xsl:when
                            test="descendant::*[contains(@syriaca-tags, '#syriaca-headword')][starts-with(@xml:lang, 'syr')]">
                            <span lang="syr" dir="rtl">
                                <xsl:apply-templates
                                    select="descendant::*[contains(@syriaca-tags, '#syriaca-headword')][starts-with(@xml:lang, 'syr')][1]"
                                    mode="plain"/>
                            </span>
                        </xsl:when>
                        <xsl:otherwise> [ Syriac Not Available ] </xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
                <xsl:otherwise>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:choose>
                                    ...
                                    <xsl:when
                                        test="t:floruit/@notBefore or t:floruit/@notAfter or t:floruit/@when or t:floruit/@from or t:floruit/@to">
                                        <xsl:choose>
                                            <xsl:when test="t:floruit/@to and t:floruit/@from">
                                                <xsl:text>between </xsl:text>
                                                <xsl:value-of
                                                  select="local:trim-date(t:floruit/@from)"/>
                                                <xsl:text> - </xsl:text>
                                                <xsl:value-of
                                                  select="local:trim-date(t:floruit/@to)"/>
                                            </xsl:when>
                                            <xsl:when
                                                test="t:floruit/@notBefore and t:floruit/@notAfter">
                                                <xsl:text>sometime between </xsl:text>
                                                <xsl:value-of
                                                  select="local:trim-date(t:floruit/@notBefore)"/>
                                                <xsl:text> - </xsl:text>
                                                <xsl:value-of
                                                  select="local:trim-date(t:floruit/@notAfter)"/>
                                            </xsl:when>
                                            <xsl:otherwise>
    ...
    <xsl:template match="t:title">
        <xsl:choose>
            <xsl:when test="@ref">
                <a href="{@ref}">
                    <xsl:sequence select="local:attributes(.)"/>
                    <xsl:apply-templates/> [<xsl:value-of select="@ref"/>] </a>
            </xsl:when>
            <xsl:otherwise>
                <span>
                    <xsl:sequence select="local:attributes(.)"/>
    ...
    <xsl:template match="t:state[@type = 'confession']">
        <xsl:if
            test="doc-available(concat('xmldb:exist://', $nav-base, '/documentation/confessions.xml'))">
            <!-- Get all ancesors of current confession (but only once) -->
            <xsl:variable name="confessions"
                select="document(concat('xmldb:exist://', $nav-base, '/documentation/confessions.xml'))//t:body/t:list"/>
            <xsl:variable name="id" select="substring-after(@ref, '#')"/>
            <li>
                <xsl:value-of select="$id"/>: <xsl:for-each
                    select="$confessions//t:item[@xml:id = $id]/ancestor-or-self::*/t:label">
                    <xsl:value-of select="."/>
                </xsl:for-each>
            </li>
        </xsl:if>
    ...
    <xsl:template match="t:state | t:birth | t:death | t:floruit | t:sex | t:langKnowledge">
        <span class="srp-label">
            <xsl:choose>
                ...
                <xsl:when test="@role">
                    <xsl:value-of
                        select="concat(upper-case(substring(@role, 1, 1)), substring(@role, 2))"/>: </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
    ...
    <xsl:template name="sources">
        ...
        <div class="well">
            ...
            <div id="sources">
                <h3>Works Cited</h3>
                <p>
                    <small>Any information without attribution has been created following the
                        Syriaca.org <a href="http://syriaca.org/documentation/">editorial
                            guidelines</a>.</small>
                </p>
                <ul>
                    ...
                    <xsl:for-each select="t:bibl">
                        <xsl:sort
                            select="xs:integer(translate(substring-after(@xml:id, '-'), translate(substring-after(@xml:id, '-'), '0123456789', ''), ''))"/>
            ...
            <ul>
                ...
                <xsl:if test="descendant-or-self::t:item[contains($current-confessions, @xml:id)]">
                    ...
                    <xsl:for-each select="t:item">
                        <xsl:if
                            test="descendant-or-self::t:item[contains($current-confessions, @xml:id)]">
                            ...
                            <li>
                                ...
                                <!-- check next level -->
                                <xsl:if
                                    test="descendant::t:item[contains($current-confessions, @xml:id)]">
                                    <ul>
                                        <xsl:for-each select="child::*/t:item">
                                            <xsl:if
                                                test="descendant-or-self::t:item[contains($current-confessions, @xml:id)]">
                                                <li>
                                                  <xsl:apply-templates select="t:label"
                                                  mode="confessions"/>
                                                  <!-- build dates based on attestation information -->
                                                  <xsl:call-template name="confession-dates">
                                                  <xsl:with-param name="place-data"
                                                  select="$place-data"/>
                                                  <xsl:with-param name="confession-id"
                                                  select="@xml:id"/>
                                                  </xsl:call-template>
                                                  <xsl:if
                                                  test="descendant::t:item[contains($current-confessions, @xml:id)]">
    ...
    <xsl:template name="confession-dates">
        ...
        <xsl:choose>
            <xsl:when
                test="$place-data//t:state[@type = 'confession' and substring-after(@ref, '#') = $confession-id]">
                <xsl:variable name="child-id"
                    select="string-join(descendant-or-self::t:item/@xml:id, ' ')"/>
                <xsl:for-each
                    select="$place-data//t:state[@type = 'confession' and substring-after(@ref, '#') = $confession-id]">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="//t:event[@type = 'attestation' and child::*[contains(@target, $ref-id)]]">
                            <!-- If there is a match process dates --> (<xsl:for-each
                                select="//t:event[@type = 'attestation' and t:link[contains(@target, $ref-id)]]">
                                <!-- Sort dates -->
                                <xsl:sort
                                    select="
                                        if (exists(@notBefore)) then
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="./@when">
                                        <xsl:choose>
                                            <xsl:when test="position() = 1">attested as early as
                                                  <xsl:value-of select="local:trim-date(@when)"/>
                                            </xsl:when>
                                            <xsl:when test="position() = last()">, as late as
                                                  <xsl:value-of select="local:trim-date(@when)"/>
                                            </xsl:when>
                                            <xsl:otherwise/>
                                        </xsl:choose>
                                    ...
                                    <xsl:when test="./@notBefore">
                                        <xsl:choose>
                                            <xsl:when test="position() = 1">attested around
                                                  <xsl:value-of select="local:trim-date(@notBefore)"
                                                />
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:if test="preceding-sibling::*">, </xsl:if>as
                                                late as <xsl:value-of
                                                  select="local:trim-date(@notBefore)"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:when>
                                    <!-- process @notAfter dates -->
                                    <xsl:when test="./@notAfter">
                                        <xsl:if test="./@notBefore">, </xsl:if>as late as
                                            <xsl:value-of select="local:trim-date(@notAfter)"/>
                                    </xsl:when>
                                    <xsl:otherwise/>
                                </xsl:choose>
                            </xsl:for-each>
                            <xsl:if
                                test="count(//t:event[@type = 'attestation' and child::*[contains(@target, $ref-id)]]) = 1">
                                <xsl:choose>
                                    <xsl:when
                                        test="//t:event[@type = 'attestation' and child::*[contains(@target, $ref-id)]][@notBefore] or //t:event[@type = 'attestation' and child::*[contains(@target, $ref-id)]][@when]">
                                        <xsl:variable name="end-date-list">
                                            <xsl:for-each
                                                select="//t:state[@type = 'confession' and contains($child-id, substring-after(@ref, '#'))]">
                                                <xsl:variable name="confession-id"
                                                ...
                                                <xsl:for-each
                                                  ...
                                                  select="@when | @notAfter"/>
                                                  <li date="{string($date)}">as late as
                                                  <xsl:value-of select="local:trim-date($date)"/>
                                                  </li>
                                                  </xsl:if>
                                                </xsl:for-each>
                                            </xsl:for-each>
                                        ...
                                        <xsl:variable name="start-date-list">
                                            <xsl:for-each
                                                ...
                                                <xsl:for-each
                                                  ...
                                                  <xsl:variable name="date"
                                                  select="@when | @notBefore"/>
                                                  <li date="{string($date)}">
                                                  <xsl:choose>
                                                  <!-- process @when dates use, local:trim-date function to trim 0 from dates-->
                                                  <xsl:when test="./@when">attested as early as
                                                  <xsl:value-of select="local:trim-date(@when)"/>
                                                  </xsl:when>
                                                  <!-- process @notBefore dates -->
                                                  <xsl:when test="./@notBefore">attested around
                                                  <xsl:value-of select="local:trim-date(@notBefore)"
                                                  />
                                                  </xsl:when>
                                                  </xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="start-date-list">
                    <xsl:for-each
                        select="$place-data//t:state[@type = 'confession' and contains($child-id, substring-after(@ref, '#'))]">
                        ...
                        <xsl:for-each
                            ...
                            <xsl:if test="@when or @notBefore">
                                ...
                                <li date="{string($date)}">
                                    <xsl:choose>
                                        <!-- process @when dates use, local:trim-date function to trim 0 from dates-->
                                        <xsl:when test="./@when">attested as early as <xsl:value-of
                                                select="local:trim-date(@when)"/>
                                        </xsl:when>
                                        <!-- process @notBefore dates -->
                                        <xsl:when test="./@notBefore">attested around <xsl:value-of
                                                select="local:trim-date(@notBefore)"/>
                                        </xsl:when>
                                    </xsl:choose>
                                </li>
                    ...
                    <xsl:for-each
                        ...
                        <xsl:for-each
                            ...
                            <xsl:if test="@when or @notAfter">
                                ...
                                <li date="{string($date)}">as late as <xsl:value-of
                                        select="local:trim-date($date)"/>
                                </li>
                            </xsl:if>
                        </xsl:for-each>
                    </xsl:for-each>
                ...
                <xsl:if
                    test="(string-length($start-date) &gt; 1) or string-length($end-date) &gt; 1">
        ...
        <xsl:variable name="name-string">
            <xsl:choose>
                ...
                <xsl:when test="@name = 'resided'">
                    <xsl:value-of select="@name"/> in </xsl:when>
                <xsl:otherwise>
                ...
                <li>
                    ...
                    <xsl:for-each-group select="t:mutual" group-by="@type">
                        ...
                        <xsl:variable name="plural-type">
                            <xsl:choose>
                                <xsl:when test="current-grouping-key() = 'building'"
                                    >Buildings</xsl:when>
                                <xsl:when test="current-grouping-key() = 'church'"
                                    >Churches</xsl:when>
                                <xsl:when test="current-grouping-key() = 'diocese'"
                                    >Dioceses</xsl:when>
                                <xsl:when test="current-grouping-key() = 'fortification'"
                                    >Fortifications</xsl:when>
                                <xsl:when test="current-grouping-key() = 'island'"
                                    >Islands</xsl:when>
                                <xsl:when test="current-grouping-key() = 'madrasa'"
                                    >Madrasas</xsl:when>
                                <xsl:when test="current-grouping-key() = 'monastery'"
                                    >Monasteries</xsl:when>
                                <xsl:when test="current-grouping-key() = 'mosque'"
                                    >Mosques</xsl:when>
                                <xsl:when test="current-grouping-key() = 'mountain'"
                                    >Mountains</xsl:when>
                                <xsl:when test="current-grouping-key() = 'open-water'">Bodies of
                                    open-water</xsl:when>
                                <xsl:when test="current-grouping-key() = 'parish'"
                                    >Parishes</xsl:when>
                                <xsl:when test="current-grouping-key() = 'province'"
                                    >Provinces</xsl:when>
                                <xsl:when test="current-grouping-key() = 'quarter'"
                                    >Quarters</xsl:when>
                                <xsl:when test="current-grouping-key() = 'region'"
                                    >Regions</xsl:when>
                                <xsl:when test="current-grouping-key() = 'river'">Rivers</xsl:when>
                                <xsl:when test="current-grouping-key() = 'settlement'"
                                    >Settlements</xsl:when>
                                <xsl:when test="current-grouping-key() = 'state'">States</xsl:when>
                                <xsl:when test="current-grouping-key() = 'synagogue'"
                                    >Synagogues</xsl:when>
                                <xsl:when test="current-grouping-key() = 'temple'"
                                    >Temples</xsl:when>
                                <xsl:when test="current-grouping-key() = 'unknown'"
                                    >Unknown</xsl:when>
                            </xsl:choose>
                        </xsl:variable>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="position() = 2">
                                ...
                                </xsl:choose>, <!-- Need to count remaining items, this is not correct just place holder -->
                                <xsl:if test="last() &gt; 2"> and <xsl:value-of
                                        select="$total-count - ($group1-count + $group2-count)"/>
                                    other places, </xsl:if>
                            </xsl:when>
                            <xsl:otherwise/>
                        </xsl:choose>
                    ...
                    <xsl:text> </xsl:text>
                    <xsl:value-of select="replace(child::*[1]/@name, '-', ' ')"/> with
                        '<xsl:value-of select="$currentPlace"/>' <xsl:text> (</xsl:text>
                    ...
                    <a class="togglelink btn-link" data-toggle="collapse" data-target="#relatedlist"
                        data-text-swap="(hide list)">(see list)</a>
                        ...
                        <xsl:for-each-group select="t:mutual" group-by="@type">
                            ...
                            <xsl:variable name="plural-type">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="current-grouping-key() = 'settlement'"
                                        >Settlements</xsl:when>
                                    <xsl:when test="current-grouping-key() = 'state'"
                                        >States</xsl:when>
                                    <xsl:when test="current-grouping-key() = 'synagogue'"
                                        >Synagogues</xsl:when>
                                    <xsl:when test="current-grouping-key() = 'temple'"
                            ...
                            <xsl:for-each select="current-group()">
                                <dd>
                                    <a href="{concat('/place/',@id,'.html')}">
                                        <xsl:value-of select="t:placeName"/>
                                        <xsl:value-of
                                            select="concat(' (', string(@type), ', place/', @id, ')')"
                                        />
                                    </a>
                                </dd>
                            </xsl:for-each>
